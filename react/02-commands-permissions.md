...
---
The root command is `/react` (alias `/re`). Use `/react help [page]` or `/react ? [page]` for generated command help. The permission root is `react.use`. `react.*` grants the full tree including shorthands.
