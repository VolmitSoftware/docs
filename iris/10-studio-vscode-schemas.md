...
2. **Open it as a world.** `/iris studio open tutorial seed=1337`
   You enter the transient world in spectator mode at its fixed generator anchor, centered on `0,0` near Y 96 and clamped to the world's build height. Studio performs no surface, hazard, fluid, collision, support, headroom, nearby-column search, or entry-area precompute. After native structure state is activated, Iris temporarily caps only the entering player's view distance at 2, delegates to Paper's asynchronous teleport, and restores the prior value when that transition succeeds or fails. A prior value of 2 or lower is left unchanged. Paper owns destination-chunk readiness. Player arrival has one absolute 10-second deadline measured from command admission, so time spent closing or waiting behind an earlier Studio transition consumes the same budget and an expired request cannot teleport late. `prepare_structure_rings` and `teleport_standard_entry` timings identify those costs separately, and successful arrival logs `Studio player <name> arrived in <milliseconds>: dimension=<key> world=<name>`. Iris refreshes the pack's canonical workspace and materializes its JSON schemas before attempting any desktop launch. A fixed seed matters — you will be comparing the same coordinates across reloads.
...
|------------|---------|--------------|
| `open <dimension> [seed=1337]` | `o` | Closes any open studio and opens the pack as a studio world. Player arrival has an absolute 10-second command-admission deadline. Blocked unless startup datapack validation is ready and the selected pack has a loadable validation result |
...
| `tpstudio` | `stp` | Delegates to Paper's asynchronous teleport at the fixed Studio entry anchor, without an Iris chunk preload or surface lookup. Standard Studio uses spectator mode. The command shares the open/close transition queue and the same absolute 10-second command-admission deadline |
...
Before an on-demand player entry requests terrain, the global scheduler claims the exact level, chunk map, generator, and state, starts placement initialization, and registers the exact concentric-ring futures. Iris lowers the Studio bootstrap gate once that state is current, then temporarily caps only the entering player's view distance at 2 and delegates the fixed entry anchor directly to Paper's asynchronous teleport. The saved view distance is restored on the player's entity scheduler after success or failure. This prevents the player's full view radius from competing with the required destination while leaving every requested chunk on Paper's canonical FULL generation path; it does not skip, blank, simplify, or substitute terrain. Paper owns the required FULL destination request, so Iris does not precompute an entry area or issue a redundant chunk request or region-thread heightmap lookup. Concentric-ring preparation may finish in the background; close and hotload still wait for its exact completion before mutating the engine. A console open performs the same structure activation but has no destination request. `tpstudio`, open, and close are serialized. The player-arrival deadline is fixed at 10 seconds from command admission, including queued close or replacement work; only the remaining time is passed to Paper, and timeout cancels or rejects the unfinished teleport so it cannot fire late. A native teleport already settled before the deadline keeps the transition barrier until its completion is applied. The final Studio callback also returns to the server scheduler before applying game rules or committing a Jigsaw session. Normal close, full hotload, and complex hotload wait up to 120 seconds for their exact aggregate before mutating or sealing the engine. A synchronous partial-start failure permanently rejects those transitions for that engine because a complete drain cannot be proven.
...
Hydrology authoring uses the ordinary dimension, region, and biome schemas. `IrisDimension.hydrology` owns `rivers` and independent `deepFluids`; `IrisDimension.riverPolicy`, `IrisRegion.riverPolicy`, and `IrisBiome.riverPolicy` resolve dimension → region → biome. River profiles are an array of unique `{id, fluidPalette}` objects. Physical settings expose routing, independent surface and underground source budgets, surface channel and hydraulic controls, ridge tunnels, mouths, grottos, and deep-fluid geometry. Policies expose placement, routing, outlet admission, profile/content pools, and geometry/routing multipliers. Regenerate schemas, validate the pack, then use **River network** to inspect only accepted feature footprints. See [36 - Rivers](/iris/36-rivers) for the complete field contract.
