...
2. **Open it as a world.** `/iris studio open tutorial seed=1337`
   You enter the transient world in spectator mode at its fixed generator anchor, centered on `0,0` near Y 96 and clamped to the world's build height. Studio performs no surface, hazard, fluid, collision, support, headroom, or nearby-column search. It asynchronously prepares hydrology for an ungenerated entry chunk during world setup. After native structure state is activated, Iris delegates to Paper's asynchronous teleport. It never changes the entering player's view distance. Paper owns destination-chunk readiness. Iris waits for Paper's teleport result without a fixed arrival deadline. `prepare_structure_rings` and `teleport_standard_entry` timings identify those costs separately, and successful arrival logs `Studio player <name> arrived in <milliseconds>: dimension=<key> world=<name>`. Iris refreshes the pack's canonical workspace and materializes its JSON schemas before attempting any desktop launch. A fixed seed matters because you will be comparing the same coordinates across reloads.
...
Modded Studio serializes open, close, and `tpstudio` per owning player. Replacing an active Studio closes and removes the prior transient dimension before creating the new pack/seed contract. Player entry uses one ticketed destination-chunk load, resolves a collision-free and fluid-free feet/head position with solid support inside the dimension bounds, and always removes the temporary ticket. Studio waits for this entry sequence without a fixed overall deadline. A failed open cleans up its transient dimension.

A rejected height or dimension-type change is not evidence that hotload is broken. Those changes are refused by design. See **Hotload rules** below.
...
| `disableTimeAndWeather` | `true` | Freezes weather and the day cycle in studio worlds and sets noon when runtime clock access is available. Unsupported runtime clocks skip the noon adjustment. Set false to let them run while authoring. Night and storm Iris spawners do not fire here until this is false or you test in a production world |
...
- Dimension `allObjectScaleFactor` edits can hotload. Save `dimensions/<dimension>.json` in the watched authoring pack, then wait for the amethyst sound and `Engine Hotloaded` action bar. New object placements use the accepted factor; existing objects retain their blocks. Explicit placement scales still override the factor, and jigsaw pieces are excluded.
...
- Native checkpoints drain their save and compression work while generation is paused, wait for the selected chunk writes, flush region storage, and verify saved terrain receipts before activation.
...
Focused child, cave, and flooded biomes retain their authored region and its cave and hydrology settings. Saved biome queries resolve those definitions from the generation snapshot. A focused biome without an authored region retains a stable neutral region in the snapshot.

Routine world-manager tasks, native mob-spawn queries, Studio scoreboard updates, and sapling replacement callbacks skip an active cutover so owner-thread checkpoint work can proceed. Queries and scoreboard updates resume under the replacement session on later ticks. Terrain checkpoints copy at most 64 chunks per batch on the owning threads, then serialize through the native save queue. Save reservations retain their order relative to ordinary autosaves. Activation still requires flushed chunk writes and verified terrain receipts. Accepted generation work must drain before activation. A watched edit keeps its place ahead of later generation requests until the admitted work drains, including cold hydrology planning. Closing Studio or shutting down cancels a watched reload that is still waiting; an admitted cutover completes before close proceeds. Explicit complex-reload requests retain their caller-specified acquisition deadline. Native structure queries allow the worker pool to run queued terrain work while waiting for overlapping structure calculations, so those waits do not starve the generation drain. Unfinished native stages can still complete after their saved terrain exists.
...
|------------|---------|--------------|
| `open <dimension> [seed=1337]` | `o` | Closes any open studio and opens the pack as a studio world. Player arrival waits for Paper's asynchronous teleport result. Blocked unless startup datapack validation is ready and the selected pack has a loadable validation result |
...
| `tpstudio` | `stp` | Delegates to Paper's asynchronous teleport at the fixed Studio entry anchor, without an Iris chunk preload or surface lookup. Standard Studio uses spectator mode. The command shares the open/close transition queue and waits for Paper's teleport result without a fixed arrival deadline |
...
The Vision map, Noise Explorer, and Image Map Studio open on the server's desktop. On a headless server, their commands report that no desktop display is available and return without opening a window. Disabled server-launched GUIs report the setting separately. These commands do not create a window on a remote player's computer.

The Vision map opens with a compact render-mode selector, world coordinates, scale, render progress, and contextual Height and River legends. Living-entity markers are hidden by default; the `Entities` toggle beside `Grid` restores or hides the red markers while player markers remain visible. When entity markers are hidden, Vision also stops requesting their periodic snapshots. Drag to pan and use the mouse wheel or trackpad to zoom in small continuous increments of about 5.7% per full wheel notch. The world coordinate beneath the live pointer remains fixed throughout zoom, and manual pan or zoom disables Follow so marker updates cannot recenter the view during the gesture. A Bukkit cursor teleport targets the player who opened the map, derives its landing Y directly from Iris's authored terrain and fluid field, then immediately delegates the centered destination to Paper's asynchronous teleport on that player's entity scheduler. It performs no separate chunk request, destination-region handoff, or world heightmap lookup; Paper owns destination-chunk readiness, and rapid clicks retain the newest requested coordinate. The displayed atlas scales immediately while one exact replacement request is coalesced after the gesture settles. Vision publishes world-aligned 64×64 biome pages before it waits for hydrology. After every visible base page is ready, one worker adds accepted river overlays to those pages. The exact-tile counter advances when each overlay is complete. Panning reuses cached base pages and resumes their unfinished overlays. Categorical modes solve uniform authored biome-domain areas as blocks and subdivide only boundaries, while Height retains its continuous natural-height sampling. The normal **Biome** view composites accepted surface hydrology content over the natural biome domain; **River network** reads the same accepted feature footprint after cross-tile composition. Its accepted labels are `headwater / source`, `surface pool`, `riffle`, `cascade`, `waterfall`, `sinkhole`, `underground pool`, `underground drop`, `coastal grotto`, `inland grotto`, `mouth`, `deep pool`, and `deep channel`. Rejected candidates stay in a separate diagnostic channel labeled `projected source`, `projected outlet`, or `projected deep fluid`, with their projected type and rejection reason, never the accepted biome or generation footprint. Pages are cached by content revision, mode, exact scale, and world coordinate, so repainting, small pans, resizes, and returning to an earlier view reuse completed work instead of sampling again. Persistent bounded workers render nearest pages first, cancel superseded pages at sample boundaries or hydrology waits, never sample on the Swing event thread, and shut down without waiting when the window closes. Hover reads the same base biome domain shown by the map. Height mode uses a normalized terrain palette instead of wrapping raw block heights around a rainbow hue wheel.

On Paper-family servers, a vanilla `/tp` command targeting an unloaded Studio chunk is canceled before the move and reissued through Paper's asynchronous entity teleport with the original command cause. This conversion remains active during generation cutover. Paper brings the destination to FULL before moving the player, so the client's teleport acknowledgement cannot force the server thread to synchronously finish a partially generated chunk. Loaded Studio destinations and non-command teleports retain their normal path. The replacement Studio listener registers before the previous manager retires, so command teleports remain intercepted throughout the swap. Command teleports into a Studio world are canceled during engine teardown. Plugin integrations are not rewritten and must use Paper's asynchronous teleport API when targeting an unloaded Studio chunk.
...
Image-map authoring uses the same desktop host and runtime interpretation contract. The workflow imports a canonical PNG, reports its dimensions and channel layout, assigns an explicit typed resource, configures coordinates and decoding, previews interpreted height, target, or mask data with chunk, region, and world-boundary overlays, and exports the source, `image-maps/<key>.json`, and dimension binding. Presets retain decoding and transform settings, while replacing the source reruns inspection and validation without rebuilding the binding. See [37 - Image Map Concepts](/iris/37-image-map-concepts) and [42 - Image Map Studio Workflow](/iris/42-image-map-studio-workflow).

Vision exposes compiled named image-map layers. The selected binding key and content revision are part of render identity, so switching between two maps cannot reuse another map's cached pages. Hover and overlays use the same coordinate transform as generation.
...
`create` with no arguments creates a project named `studio` from the `example` template. `create <name>` uses the same template. `create <name> <template>` picks another. `open <pack> [seed]` defaults the seed to `1337`. Player `open` and `tpstudio` use the serialized safe-entry sequence above, without a fixed overall arrival deadline.
...
Studio snapshot preparation copies files with up to four workers, then flushes every destination after all copies finish. Directory flushes run concurrently at each depth and finish before parent directories are flushed. An exact copied snapshot can reuse the source validation result when its content fingerprint and validation context match. Pack-validation fingerprints ignore Finder `.DS_Store` metadata at every directory depth, so those metadata changes do not invalidate an otherwise unchanged pack. Fresh history construction reuses the copied-stage verification; opening existing history and resolving an active pack still verify its content. File and directory durability checks complete before publication. The `prepare_world_pack` timing includes pack installation before generator construction. Generator startup reuses its loaded dimension when the verified active pack is unchanged; changed or closed targets are reloaded.
...
For an ungenerated destination chunk, Normal Studio starts its hydrology task during generator injection, in parallel with world setup. It uses the opening runtime and participates in reload and shutdown tracking. This replaces broad spawn prefetch for Normal Studio and does not generate terrain or change the destination.

Standard Studio keeps a small, ten-minute in-process cache of successfully completed hydrology tiles and a bounded persistent cache for the entry and initial-pregeneration neighborhood. Persistent entries live under `packs/<key>/.iris/studio-hydrology/<identity>/` as atomically replaced compressed files. Reuse requires the same complete authoring-pack snapshot, validation context, dimension key, seed, world height, hydrology settings, and tile coordinate. Each saved final tile includes regional course ownership. It stores local diagnostic candidates; complete regional rejection details are resolved separately when an inspector requests them. Cold entry skips fine planning for basins whose conservative route bounds cannot affect its required terrain, while preserving full basin conflict checks. Loading it restores resolved owner state directly, without sampling terrain or rebuilding regional routes to recover that ownership; adjacent tiles can reuse the restored local owner draft. Iris fingerprints the full visible packs directory before and after runtime construction; a concurrent edit disables reuse, old identities are ignored, invalid files are replanned, failed or interrupted plans are never shared, and closing or invalidating a runtime cannot publish late work. The first cold plan for a new identity still pays the planning cost before it can populate this cache. Object and Jigsaw Studio do not use it.

Opening Studio prepares the authoring world before moving the player. Native structure-ring activation waits for its actual completion without a fixed 30-second deadline. If preparation fails, Iris drains tracked ring work before closing the temporary world and reports the failure.

Studio cleanup waits for the actual world unload and generator shutdown. After two minutes, unfinished cleanup logs a warning and continues waiting without restarting the server. A completed cleanup failure retains the world maintenance state and blocks further Iris world mutations. Resolve the reported error, then restart manually to clear that recovery state. Iris keeps temporary world folders until shutdown is safe and queues failed cleanup for the next startup.

Object and Jigsaw Studio use flat authoring floors and the `minecraft:plains` biome. Their biome and base-height queries do not sample the pack's normal terrain or hydrology. Native heightmaps read the actual floor and placed object blocks. Object Studio restores saved block-entity data on chunk load before accepting gallery saves. Both skip normal generation-cache warmup, spawn hydrology prefetch, native structure activation, and imported-feature placement. Pack validation, runtime construction, and object loading still occur.

`OBJECT_BUFFET` also uses flat terrain queries and heightmaps from actual blocks, and skips native structure and imported-feature placement. Standard Studio without `OBJECT_BUFFET` retains the complete terrain and hydrology pipeline for the selected pack and seed.
...
Folders with schemas: `dimensions`, `regions`, `biomes`, `generators`, `image-maps`, `loot`, `entities`, `spawners`, `markers`, `blocks`, `expressions`, `mods`, `structures`, `jigsaw-pools`, `jigsaw-pieces`. The object, source-image, and matter loaders return false from `supportsSchemas()` because their content is binary, so `objects/`, `images/` and `matter/` get no schema. Typed JSON under `image-maps/` does receive a schema.
...
Biome Buffet sorts supported pack biomes by load key and places them from chunk `(0, 0)` along positive X, then positive Z. Each cell treats its selected biome as land and uses its authored owner region, or a deterministic neutral owner when no region references it. Cells outside the layout contain a barrier floor. Terrain and saved biome queries use the same coordinate selection; changing cells neither changes `focus` nor hotloads the pack.
...
Hydrology authoring uses the ordinary dimension, region, and biome schemas. `IrisDimension.hydrology` owns `rivers` and independent `deepFluids`; `IrisDimension.riverPolicy`, `IrisRegion.riverPolicy`, and `IrisBiome.riverPolicy` resolve dimension → region → biome. River profiles are an array of unique `{id, fluidPalette}` objects. Physical settings expose routing weights, independent surface and underground source budgets, surface channel, bank, flow and mouth controls, grottos, and deep-fluid geometry. Policies expose placement, routing, outlet admission, profile/content pools, and geometry/routing multipliers. Regenerate schemas, validate the pack, then use **River network** to inspect only accepted feature footprints. See [36 - Rivers](/iris/36-rivers) for the complete field contract.
