...
2. **Open it as a world.** `/iris studio open tutorial seed=1337`
   You are teleported into the transient world's fixed 5×5 Studio landing lobby centered on chunk `0,0`; chunks outside that lobby use the pack's complete generator. Iris refreshes the pack's canonical workspace and materializes its JSON schemas before attempting any desktop launch. A fixed seed matters — you will be comparing the same coordinates across reloads.
...
Four values are pinned for the life of the world and cannot hotload. They are the dimension type key (derived from the dimension load key), min height, total height, and logical height. Editing `dimensionHeight`, `logicalHeight`, `environment`, or the dimension file name means closing and reopening Studio. See [11 - Dimensions](/iris/11-dimensions).
...
| `tpstudio` | `stp` | Loads or reuses the persistent synthetic Studio entry, resolves its safe landing, and teleports there in creative. It shares the open/close transition queue and reports a timeout after 10 seconds |
...
The Vision map opens with a compact render-mode selector, world coordinates, scale, render progress, and contextual Height and River legends. Drag to pan and use the mouse wheel to zoom around the cursor. Iris renders the nearest visible tiles first as a deliberately low-resolution preview, then progressively replaces them with settled detail. All normal map modes sample the base, natural terrain streams and do not evaluate the river network; select **River network** when river computation and diagnostics are needed. A pan, zoom, mode change, hotload, or engine replacement invalidates older work; stale tiles cannot replace the current view. Painting and hover display consume completed snapshots only, and an idle window performs no terrain sampling. Height mode uses a normalized terrain palette instead of wrapping raw block heights around a rainbow hue wheel. River mode keeps the runtime river classifications and draws wet channels, mouths, banks, dry channels, and the no-river field distinctly.

Noise Explorer shows its window before scanning the generator catalog and loads a pack generator only when selected. Its seed field starts from the command seed; **Apply** rebuilds the selected sampler deterministically, and the seed is mixed once per rebuild rather than once per pixel. Drag to pan, use the mouse wheel to zoom around the cursor, and choose the signed, terrain, or grayscale palette from the toolbar. The status strip reports source, coordinates, scale, sampled range, and render progress, including invalid or out-of-range values. Sampling runs in bounded row-major background work: a preview capped at about 24,000 samples completes first, then its measured throughput sizes one bounded refinement pass to target roughly 500 milliseconds. Superseded work is cancelled, and an unchanged idle view is not sampled again.
...
Ordinary Studio completes its runtime and generator preparation before reporting ready. A console-issued `STANDARD` open does not request, load, or retain the landing chunk. A real player open, or a later `tpstudio`, loads the persistent synthetic entry on demand. For `NORMAL` Studio mode, that entry is a centered, lit 5×5 lobby from chunk `-2,-2` through `2,2`, generated without pack terrain, mantle, structures, carvers, or decoration. Its footprint covers the terrain-bearing portion of Paper 26.2's FULL entry-chunk dependency pyramid while a player is entering. The remaining BIOMES and native structure-ring queries use an isolated natural-biome path without river, cave, or height evaluation. Production worlds, Jigsaw Studio, Object Studio, biome and region buffet modes, and Studio chunks outside the lobby keep their complete generator. The lobby persists only inside the transient Studio world, which is deleted on close. Studio may launch the pack workspace and preserves native structures for generation previews after bootstrap. On Paper 26.2, WorldInit publishes the filtered native-structure placement state once but leaves it uninitialized while native starts, locates, and object-collision volume queries are gated. Injection verifies that Paper's canonical chunk-generator getter owns the new Iris generator before native structure state is published.

For an on-demand player entry, Iris requests the exact FULL landing chunk and then holds it with a plugin ticket through safe-entry resolution and native teleport settlement. It releases that ticket as soon as the teleport settles; a console open has neither the chunk request nor the ticket. The global scheduler separately claims the exact level, chunk map, generator, and state, starts placement initialization, and registers the exact concentric-ring futures. Iris waits for that real native completion before lowering the Studio bootstrap gate or reporting the world ready, so a later player teleport cannot inherit an unfinished ring search. `tpstudio`, open, and close are serialized: a teleport that has not reached Paper by its 10-second deadline cannot fire late, while one already delegated keeps the transition barrier until Paper settles it. The final Studio callback also returns to the server scheduler before applying game rules or committing a Jigsaw session. Normal close, full hotload, and complex hotload wait up to 120 seconds for their exact aggregate before mutating or sealing the engine. A synchronous partial-start failure permanently rejects those transitions for that engine because a complete drain cannot be proven.
...
Paper-family on-demand entry chunks are requested through the urgent asynchronous chunk API before Iris adds the temporary plugin ticket. Folia keeps its nonblocking ticket bootstrap and confirms the owning region before entry resolution. The ticket is released after teleport settlement instead of retaining the entry for the life of the Studio world. This prevents Paper from starting delayed radius-3 dependency generation after Studio has already reported ready, and prevents Studio close or Spark statistics queries from stalling behind that queued chunk work. A console `STANDARD` open has no entry load to delay either operation. If a player-issued open fails while its exact request remains active, Iris reports the failure without unloading or closing the generator. It rejects another Studio open until cleanup succeeds. It queues the transient world for deletion at the next clean startup if it remains active for another 120 seconds. If a legitimate registry change has already queued a terminal server restart, failed-open cleanup does not compete for a live lifecycle lease. Any materialized transient state is queued for startup deletion instead. Closing a Studio stops Iris engine maintenance at `WorldUnloadEvent`. It waits for the raw backend unload completion and any tracked native ring preparation before sealing the generator. The 26.2 noise pipeline keeps its generation lease through terrain and heightmap completion. Forced process termination cannot drain in-process ring futures.

Every Bukkit Studio open writes one `[Studio timing]` line per lifecycle phase. The line names the transient world, `standard` or `jigsaw` kind, phase duration, and cumulative duration where available. The measured phases separate loaded-runtime reuse, external-datapack recovery, compiler-input fingerprinting, datapack compilation/publication, generator preparation, Bukkit world creation, optional entry-chunk loading and safe-entry resolution, standard teleport, and finalization. Studio engine timing additionally separates prefetch loading, runtime construction, and the generation-cache warm or its Jigsaw-only skip. A slow open can then be correlated with a profiler capture.
