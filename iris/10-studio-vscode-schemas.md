...
2. **Open it as a world.** `/iris studio open tutorial seed=1337`
   You are teleported onto safe real pack terrain in the transient world near chunk `0,0`. Iris prefers a dry collision-supporting floor; when the exact entry chunk has no dry candidate, it enters immediately above a clear water surface instead of fabricating a platform or failing on an ocean seed. Before requesting that complete entry chunk, Iris finishes native concentric-ring placement from cached natural-biome suitability; `prepare_structure_rings` and `load_entry_chunk` timings identify those costs separately. Iris refreshes the pack's canonical workspace and materializes its JSON schemas before attempting any desktop launch. A fixed seed matters — you will be comparing the same coordinates across reloads.
...
| `tpstudio` | `stp` | Urgently loads the real Studio entry chunk, resolves a safe landing on its generated terrain, and teleports there in creative. It shares the open/close transition queue and reports a timeout after 10 seconds |
