...
On Bukkit-family servers, Iris delivers the resulting text through the shared VolmLib component pipeline for command chat, action bars, rich components, and plugin logging. Player colors, RGB, decorations, and authored click or hover events survive. Console, RCON, and plain-server fallbacks receive plain text instead of raw `§` markers. Iris's modded adapters retain their native component and dedicated-server delivery paths.
