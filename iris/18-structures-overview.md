...
`/iris jigsaw` is Bukkit-only and player-only. Fabric, Forge, and NeoForge generate saved jigsaw structures perfectly well. They just have no authoring commands. Author on a Bukkit server and deploy the pack.
...
| One authored graph that must also work as a vanilla 26.2 datapack | `compatibility: VANILLA_PORTABLE`, then `/iris jigsaw export` | [21 - Jigsaw Structures](/iris/21-jigsaw-structures) |
