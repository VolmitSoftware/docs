...
**Neighbors.** `forbiddenCollisions` lists object keys this object refuses to intersect. Iris evaluates collision-aware placements in one canonical source-chunk order, including a bounded predecessor halo large enough for the complete transformed object reach, then replays each immutable source plan into every destination chunk it touches. Chunk request order and parallel generation therefore cannot change which object wins. If any transformed block of an earlier source with a forbidden key intersects this object's transformed bounding box, the attempt is dropped. Rotation, rotated translation, warp reach, ceiling inversion, and random Y translation are included. `allowedCollisions` names exceptions that win over the forbidden list. Both lists are empty by default; packs without collision rules use the direct placement path.
