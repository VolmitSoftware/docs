...
Iris does not download packs at startup. `/iris download pack=overworld` installs the hardcoded Overworld beta-release ZIP into `packs/`; restart afterward before opening Studio or creating a world (see [02 - Getting Started](/iris/02-getting-started), [25 - Pack Management](/iris/25-pack-management)).
...
**What you should see.** With the workspace open, the editor should autocomplete `generator` values against the fork's real generator keys and flag a typo in `derivative` immediately. If it does not, run `/iris studio update dimension=my-overworld` (see [10 - Studio & VSCode Schemas](/iris/10-studio-vscode-schemas)).
...
Reuse `snippet/decorator/*` and `snippet/style/*` by string reference as in [24 - Pack Mods & Snippets](/iris/24-pack-mods-snippets). Existing examples: `biomes/vanilla/old_growth_birch_forest.json` and the dimension's ore `chanceStyle` fields.
