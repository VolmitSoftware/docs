...
Two more keys affect these menus and are documented in [01 - Installation & Configuration](/adapt/01-installation-configuration): `guiBackButton` (default `true`, reserves the navigation row in the 9-wide menus) and `customModels` (default `true`, enables `models.toml` lookups).
