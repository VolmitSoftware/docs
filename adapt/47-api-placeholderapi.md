...
---
Adapt registers a PlaceholderAPI expansion under the identifier `adapt` whenever PlaceholderAPI is installed. Every key looks like `%adapt_<path>%`. `<path>` is one to four dot-separated segments of lowercase letters, digits, and hyphens. You do not register anything. You do not compile against Adapt. Install PlaceholderAPI, install Adapt, and the keys resolve.

There is no underscore anywhere in a path. PlaceholderAPI uses `_` to terminate the expansion identifier. `%adapt_player_level%` is not a typo Adapt can recover from. It is a request for a different expansion entirely. The separator inside a path is always `.`.

Values come from a snapshot that each player's own Adapt tick republishes roughly once a second. They never come from live game state. That makes resolution cheap enough to put in a scoreboard that updates every tick. A value can be up to a second behind. The answer is also safe to ask for from any thread.

Three things can come back. The literal text means the path is not a key.
`---` means the path is a key but there is nothing to show. The last outcome is
a value. Adapt never invents a plausible-looking zero for a path it does not
recognize.
...
Paths are lowercased before resolution, so `%adapt_Player.Level%` works. Nothing else about a path is forgiving. Attribute matching is exact and never by prefix. A numeric argument must be one to four unsigned digits. `can-claim-nextra`, `can-claim.next`, `can-claim.`, `cost-to.x`, `cost-to.-1`, `has-level.five`, and `has-levelx.5` all come back as the literal.
...
The catalogue snapshot, which holds skills, adaptations, mutations, and their costs, is separate and server-wide. It is republished whenever the skill registry changes. It is available to any reader, including one with no player snapshot at all. That is why `catalog.*` answers for offline players and for the console.

## Threading

Any thread. This is the one surface in Adapt where that is the correct answer. The per-player store is a `ConcurrentHashMap` of immutable records holding immutable maps. The catalogue is a single `volatile` reference to an immutable object. Resolution reads those records. The only Bukkit call on the path is `OfflinePlayer.getUniqueId()`, which Adapt uses to key the store. No entity is read. No world is touched. No lock is taken. PlaceholderAPI may resolve on the main thread, a region thread, or an async chat or scoreboard task. All three are safe.

Publication is the only write. It happens on the thread that owns the player's data. A reader that races a publication sees either the whole previous snapshot or the whole new one. It never sees a mixture.

## What you can ask for

Server and catalogue keys need no player and answer for the console. Player keys need a published snapshot. Skill keys take a skill id. Adaptation keys take a full adaptation id such as `rift-blink` or `pickaxe-veinminer`. Mutation keys take a mutation id such as `gale-lung`. Every key, id, and metric is listed in the [Reference](#server-and-catalogue-keys).

Two behaviors catch people out.

`can-use` is not a permission check. It reads three things (learned level, adaptation enabled, skill enabled) and nothing else. It ignores `adapt.use.*`, world blacklists, game mode, protection plugins, usage conflicts, and any registered [`AbilityUsePolicy`](/adapt/43-api-ability-use-policy). A player can show `can-use = true` here and still have the adaptation refuse to fire. Read it as "owned and switched on", not "will work if they try".

`can-claim.<n>` follows the same rules the GUI does. A target equal to the current level is always `true`. A target above it is `true` only when the player has enough ability power and enough knowledge in the owning skill for the whole jump. A target below it is a refund. That is `false` for a permanent adaptation and otherwise `true` whenever the current level is above zero. Refunds do not need spare power. The target is clamped to `0..max-level`. `can-claim.9999` answers about the maximum level.

## Discovering keys at runtime

Adapt publishes its key list to PlaceholderAPI. Anything that reads an expansion's advertised placeholders sees it. The list contains every exact key on this page plus three group markers, `skill.*`, `adaptation.*`, and `mutation.*`. The per-skill, per-adaptation, and per-mutation paths are resolved dynamically from the catalogue rather than enumerated one by one.
...
There is nothing to configure. The expansion has no keys in `adapt.toml`, no toggle, and no per-key switches. `%adapt_player.max-level%` reads the `experienceMaxLevel` core setting. The skill and adaptation `enabled` keys read their own components' config. The placeholder layer itself is not configurable.
...
`name`, `enabled`, and `adaptations` come from the catalogue rather than the player. They still need a player snapshot to resolve, because the resolver checks for one before it answers anything under `skill.`.
...
| `name` | the adaptation's display name, color codes stripped |
...
Every key in that table except `available` answers `---` when there is no
player snapshot. It also answers `---` when the mutation runtime has not
published for this player. Check `available` first.
...
`state` is the `MutationState` enum constant lowercased. The enum may gain constants. Treat an unrecognized value as unknown rather than switching exhaustively on the seven above. These six answer `---` when the mutation runtime is unavailable for the player, and the literal for any other attribute name.
