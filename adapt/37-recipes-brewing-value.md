...
Vanilla gunpowder and dragon's breath conversion remains available for Adapt potions after brewing. See [Skill - Brewing](/adapt/15-skill-brewing) for each owning adaptation's level, duration, amplifier, and enable setting.
...
`value.valueMultipliers` keys are Bukkit material names such as `DIAMOND_ORE`. An absent key uses multiplier `1`; the default map and core value settings are listed in [Installation & Configuration](/adapt/01-installation-configuration).
