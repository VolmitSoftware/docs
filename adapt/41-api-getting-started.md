...
|---------------------------------------------------------------------|------------------------------------------------------------------|----------|
| answer "may this player use this adaptation right now", with a reason | `AbilityUsePolicy` (ServicesManager)                             | [43 - API - Ability Use Policy.md](/adapt/43-api-ability-use-policy) |
| price an activation, or charge for it from your own economy          | `AbilityCostProvider` (ServicesManager)                          | [44 - API - Ability Cost.md](/adapt/44-api-ability-cost) |
| gate block/entity actions on claims or regions                       | `Protector` (`ProtectorRegistry`)                                | [46 - API - Protection.md](/adapt/46-api-protection) |
| supply region XP, power, and temporary adaptation grants             | `RegionPolicySource` (`RegionPolicyService`)                     | [46 - API - Protection.md](/adapt/46-api-protection#region-policy-source>) |
| watch or veto activation and adaptation events                       | the ability events and `AdaptEvent` family                       | [45 - API - Events.md](/adapt/45-api-events) |
| inspect catalogue objects or change a learned level transactionally  | `SkillRegistry`, `AdaptationLearningTransaction`                 | [42 - API - Skills & Adaptations.md](/adapt/42-api-skills-adaptations) |
| inspect mutation catalogue/snapshot value objects                    | `MutationCatalog`, `MutationSnapshot`                            | [48 - API - Mutations.md](/adapt/48-api-mutations) |
| look up online data or award progression through production paths    | `AdaptServer`, `XP`                                              | [49 - API - Player Data, XP & World.md](/adapt/49-api-player-data-xp-world) |
| build recipes/effects or read diagnostics                            | recipe, FX, telemetry, projectile, and value helpers             | [50 - API - Recipes, FX, Telemetry & Utilities.md](/adapt/50-api-recipes-fx-telemetry-utilities) |
| read levels, power, and mutations as text                            | the `%adapt_…%` PlaceholderAPI expansion                         | [47 - API - PlaceholderAPI.md](/adapt/47-api-placeholderapi) |
...
itself from the event rather than by walking a skill's collection. [45 - API - Events.md](/adapt/45-api-events) lists the safe
accessors.
...
- **`AbilityCostProvider.commit` and `refund`.** `commit` is on the owning thread for every charge the
  ...
  [44 - API - Ability Cost.md](/adapt/44-api-ability-cost#threading>).
- **The older `AdaptEvent` family** carries an async flag computed from the firing thread and can genuinely
  be asynchronous — see [45 - API - Events.md](/adapt/45-api-events#threading>).
...
and are safe to resolve from any thread. See [47 - API - PlaceholderAPI.md](/adapt/47-api-placeholderapi).
...
[`AbilityUsePolicy`](/adapt/43-api-ability-use-policy).
...
|---------|---------|----------|
| `[abilityApi]` | the whole ability API: master switch, failure modes, fault limit, slow-call threshold | [43 - API - Ability Use Policy.md](/adapt/43-api-ability-use-policy#configuration>), [44 - API - Ability Cost.md](/adapt/44-api-ability-cost#configuration>) |
| `[protectorSupport]` | which built-in protectors are on by default | [46 - API - Protection.md](/adapt/46-api-protection#configuration>) |
| `[protectionOverrides]` | per-adaptation protector overrides | [46 - API - Protection.md](/adapt/46-api-protection#configuration>) |
