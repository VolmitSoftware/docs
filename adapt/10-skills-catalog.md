...
Movement. Sprint, swim, climb, and stay airborne to level it. Spend knowledge on wall jumps, air dashes, slides, and rolls.

### Architect (`architect`)

Building. Level it by placing blocks. It pays builders wands, chalk guides, dissolving scaffolds, wool elevators, and an undo.

### Axes (`axes`)

Chopping and axe combat. Fell a whole tree from the base log. Vein-mine wood and leaves. Throw your axe. Shred armor on hit.

### Blocking (`blocking`)

Shields. Level it by blocking hits. Then unlock counters, reflected projectiles, perfect guards, and recipes for chainmail, saddles, and horse armor.

### Brewing (`brewing`)

Potions. Level it by brewing. Then run stands hotter, stretch potion durations, and unlock bottled effects vanilla does not give.

### Chronos (`chronos`)

Time. Level it by moving, sleeping, and surviving. A clock speeds the payout. Rewind a saved moment. Freeze projectiles. Speed nearby furnaces and crops.

### Crafting (`crafting`)

The crafting grid. Level it by crafting. Then salvage items, craft in bulk, carry portable workstations, and stamp gear with a signature.

### Discovery (`discovery`)

Exploring and collecting experience. It pays for first sightings. It reveals structures and hidden chests. It shows names, health bars, and damage numbers.

### Enchanting (`enchanting`)

The enchanting table, anvil, and grindstone. You get cheaper anvil work. You can preview and reroll offers. Recover enchantments onto books. Soul-link gear so it survives death.

### Excavation (`excavation`)

Shovels and soft ground. Haste lasts long enough to finish the block. Ore glows through dirt. Dig whole planes. Burrow down. Find treasure in gravel.

### Herbalism (`herbalism`)

Farming and gathering. Growth auras, harvest-and-replant in one click, composting cascades that mature a field, and a hunger bar that soaks damage.

### Hunter (`hunter`)

Killing mobs. Most boons trigger when you take a hit, not when you land one. Those boons cost hunger.

### Kinetics (`kinetics`)

Momentum, maces, and spears. Higher jumps, springier landings, meteor dives into a mace smash, shockwaves, and spears that hit harder as you charge.

### Nether (`nether`)

Surviving the Nether. Fire and wither resistance, walking on lava, better piglin bartering, faster netherrack mining, and striders that behave.

### Pickaxes (`pickaxe`)

Mining. Vein-mine ore. Autosmelt what you break. See ore through stone. Get haste in deepslate and obsidian. Carry a pickaxe that refuses to break.

### Ranged (`ranged`)

Bows and crossbows. Farther and faster shots, piercing, ricochets off walls, a dotted trajectory preview, and arrows that lock on and curve to the mark.

### Rift (`rift`)

Ender items and teleporting. Blink to where you look. Bind a recall gate. Open containers from anywhere. Survive a lethal hit by blinking out.
...
Sword combat. Bleed and poison on your strikes, dual-wield stances, ripostes off a block, sprinting lunges, and extra damage against hurt targets.

### Taming (`taming`)

Pets. Tougher, faster, harder-hitting companions, a pack that focuses what you mark, mounted combat bonuses, and pets that dive in front of arrows.

### TragOul (`tragoul`)

Blood. Level it by taking damage and living at low health. Thorns, life siphoned from hits, corpse explosions, skeletal servants, and a killing blow that leaves you at 1 HP.

### Unarmed (`unarmed`)

Fists. Combo chains, disarms that knock a weapon from a player's hand, shockwave claps, grapples that throw people, and damage that climbs as armor drops.
...
Adapt declares 312 adaptation types. 311 register on a plain server. The twelfth Axes entry, Iris Feller, only registers when the Iris tree-feller link is available.

`SkillArchitect` sets its icon twice. The second call is `SMITHING_TABLE`. The menu shows `SMITHING_TABLE`, not the `IRON_BARS` it sets first.
...
| Agility (`agility`) | `agility-slipstream-slide` | Slipstream Slide | Tap sneak while sprinting to enter a prone slide. You shed ground friction and keep momentum through 1-block gaps. |
...
| Agility (`agility`) | `agility-marathoner` | Marathoner | Sprinting drains less saturation. You can run farther before hunger stops you. |
| Agility (`agility`) | `agility-kip-up` | Kip-Up | Jump right after taking a hit. Knockback becomes recovered momentum and a speed burst. |
...
| Architect (`architect`) | `architect-placement` | Builders Wand | Sneak while holding a block that matches the surface you look at. You place multiple blocks across that surface at once. You may need to move slightly to refresh the placement preview. |
...
| Architect (`architect`) | `architect-scaffolder` | Scaffolder | Sneak-place blocks as temporary scaffolds. They dissolve on their own and refund the block to you. |
| Architect (`architect`) | `architect-supply-line` | Supply Line | When the stack in your hand runs out, it refills from shulker boxes or bundles in your inventory. |
| Architect (`architect`) | `architect-steady-hands` | Steady Hands | While bridging over open air you take no knockback. You shrug off falls and place with a steadier rhythm. |
| Architect (`architect`) | `architect-chalk-line` | Chalk Line | Craft persistent Chalk Wands from one Stick and one String in different orientations. Level 1 unlocks straight lines, level 2 polylines, level 3 circles, and level 4 arcs. Each level immediately reveals its new crafting recipe in the vanilla recipe book. Private block guides appear only while that wand is held. Sneak-click the air to clear its saved plan. |
...
| Axes (`axes`) | `axe-iris-feller` | Iris Feller | Sneak-break an Iris tree with an axe. Keep sneaking and keep that original axe held while the tree erodes outward. Each successfully eroded log costs hunger. The run stops if you stop sneaking, drop the original axe, or lack hunger for the next log. Accepted runs start an activation cooldown. |
...
| Blocking (`blocking`) | `blocking-counter-guard` | Counter Guard | Each blocked hit builds shield stacks. The action bar briefly shows current/max. Your next proc consumes stacks to reflect damage. |
...
| Chronos (`chronos`) | `chronos-time-bottle` | Time In A Bottle | Carry a temporal bottle that stores time. Spend it to accelerate timed blocks, growables, and Ageable entities such as baby animals. Its shapeless recipe uses a Swiftness Potion, a Clock, and a Glass Bottle. |
| Chronos (`chronos`) | `chronos-aberrant-touch` | Aberrant Touch | Melee attacks apply stacking slowness at the cost of hunger, with strict PvP caps. Targets root at 5 stacks. |
| Chronos (`chronos`) | `chronos-instant-recall` | Instant Recall | Rewind to a recent snapshot with health and hunger restored. The clock and part of your remaining health are spent. The recall never kills you. |
...
| Chronos (`chronos`) | `chronos-stasis-field` | Stasis Field | Sneak and right click with an amethyst shard to deploy a stasis bubble. The bubble freezes projectiles in midair and locks down mobs inside. The shard is consumed on cast. |
| Chronos (`chronos`) | `chronos-rewind` | Rewind | Sneak and swap hands to mark a moment in time. Do it again within the window to snap back with health and hunger restored. Each rewind costs hunger. |
| Chronos (`chronos`) | `chronos-borrowed-time` | Borrowed Time | A portion of incoming damage is deferred. It drains back once per second over the following seconds. |
...
| Chronos (`chronos`) | `chronos-accelerate` | Accelerate | Passively accelerate time around you. Nearby crops grow. Furnaces, smokers, blast furnaces, and brewing stands fast-forward. |
| Chronos (`chronos`) | `chronos-hourglass-guard` | Hourglass Guard | A killing blow leaves you at half a heart instead. You get brief invulnerability and nearby enemies slow. The cooldown is long. |
...
| Crafting (`crafting`) | `crafting-signature` | Artisan's Signature | Items you craft carry your signature. Villagers offer better trades while you carry your signed goods. |
...
| Discovery (`discovery`) | `discovery-xp-resist` | Experimental Resistance | Spend an immediately visible vanilla-level charge to mitigate a hit. The hit must be one whose effective damage would drop you below 5 hearts or kill you. |
...
| Discovery (`discovery`) | `discovery-insight` | Insight | Study creatures at a glance. The entity you look at shows its name and health bar above its head. Tameable creatures show live speed, jump, and attack stats. Your hits show floating damage numbers with crits in orange. |
...
| Discovery (`discovery`) | `discovery-sixth-sense` | Sixth Sense | A compact HUD names the nearest supported structure within up to 500 blocks. It shows direction and distance. It clears inside the structure. |
...
| Excavation (`excavation`) | `excavation-burrow` | Burrow | Sneak-right-click soft ground with a shovel to rapidly dig straight down. The dig stops before hazards. Each burrow costs hunger and tool durability. |
...
| Herbalism (`herbalism`) | `herbalism-replant` | Harvest & Replant | Right click a crop with a hoe to harvest and replant it. |
| Herbalism (`herbalism`) | `herbalism-hungry-shield` | Hungry Shield | Take damage to your hunger before your health. More damage types are covered as it levels up. |
...
| Herbalism (`herbalism`) | `herbalism-compost-cascade` | Compost Cascade | Sneak-right-click a composter to consume nearby drops, harvest and replant mature crops, compost your inventory, and spend the compost maturing nearby crops. Leaves are consumed only when enabled in the config. |
| Herbalism (`herbalism`) | `herbalism-rooted-footing` | Rooted Footing | Permanent passive. Protect farmland. Convert part of fall damage into hunger while on natural ground. |
...
| Kinetics (`kinetics`) | `kinetics-rubber-soul` | Rubber Soul | Your landings carry spring. Bouncy blocks send you higher. Every landing keeps more momentum. |
| Kinetics (`kinetics`) | `kinetics-soft-catch` | Soft Catch | Soft and springy blocks break your fall. A fresh bounce grants a grace window. |
...
| Kinetics (`kinetics`) | `kinetics-heavy-frame` | Heavy Frame | Plant your feet while sneaking with a mace or spear. You get heavy knockback resistance at the cost of speed. |
| Kinetics (`kinetics`) | `kinetics-mass-shift` | Mass Shift | Sneak and swap hands. Look up for persistent Titan form. Look down for persistent Pocket form. Look level to return to normal. Titan grants 20% damage and health with Slowness I. Pocket trades 20% damage and health for Speed I. |
...
| Ranged (`ranged`) | `ranged-trajectory-sight` | Trajectory Sight | Sneak or draw a ranged weapon to preview projectile flight as a dotted line with a ringed impact marker. With a Heartseeker lock active, the preview shows the arrow's curved seeking path to the mark instead. |
...
| Ranged (`ranged`) | `ranged-heartseeker` | Heartseeker | Draw a bow while looking at a creature to lock on. The mark glows red for you. The arrow curves around obstacles to the mark no matter where you aim. Piercing levels and Ricochet Bolt bounce capacity add seeking passes. The arrow punches through its target, exits the far side, then bends toward a fresh nearby target. With no new target it keeps flying forward. Ricochet passes keep reflection, speed, damage, and rewards when seeking arrows strike blocks. Every seeking shot puts your bow on a short cooldown. |
| Rift (`rift`) | `rift-resist` | Rift Resistance | Gain Resistance when using Ender Items and Abilities. |
| Rift (`rift`) | `rift-access` | Remote Access | Craft a Reliquary Portkey (ender pearl + compass). Bind it to a container. Use it to open that container from anywhere. |
| Rift (`rift`) | `rift-enderchest` | Easy Enderchest | Click while holding an ender chest to open your ender chest without placing it. |
| Rift (`rift`) | `rift-gate` | Rift Gate | Craft a recall gate (Emerald + Amethyst Shard + Ender Pearl). Sneak-left-click it to bind your location. Right-click to teleport back after a 5 second channel. Sneak-left-click into the air to unbind. |
| Rift (`rift`) | `rift-blink` | Rift Blink | Double-jump to blink toward where you look. Aim at ground to land there. Aim at a ledge to mantle onto it. Aim into open air to dash. Successful blinks consume no pearl. They deal normal ender pearl damage that decreases by level. Sneak while blinking to phase straight through walls and obstacles. |
...
| Rift (`rift`) | `rift-void-skin` | Void Skin | Lethal damage blinks you to a nearby safe spot instead of killing you. If no nearby spot exists, the current world's spawn is used. You get brief resistance. The cost is an ender pearl and a long cooldown. Leveling shortens the cooldown and extends the protection. |
| Rift (`rift`) | `rift-pearl-rebound` | Pearl Rebound | A plain thrown ender pearl bounces once instead of teleporting at the first surface it hits. The rebound steers toward your crosshair. Then it teleports normally at its next impact. Pearl landing damage is also reduced. Leveling improves both reduction and steering. |
...
| Seaborne (`seaborne`) | `seaborne-coral-gardener` | Coral Gardener | Coral you place survives out of water far longer. Bonemeal grows coral. Reef blocks grant bonus XP. |
| Seaborne (`seaborne`) | `seaborne-deep-salvager` | Deep Salvager | Underwater containers appear as private aqua glowing block displays. They reward bonus treasure the first time you open them submerged. |
| Seaborne (`seaborne`) | `seaborne-ink-veil` | Ink Veil | Taking damage underwater bursts a visible ink cloud. Hostiles are blinded. Drowned and guardian targeting is blocked for a short time. |
...
| Seaborne (`seaborne`) | `seaborne-fish-whisperer` | Fish Whisperer | Fish school toward you. Dolphins and axolotls assist your hunts. You fish with a permanent Luck of the Sea tier. |
| Seaborne (`seaborne`) | `seaborne-hydro-jet` | Hydro Jet | Tap sneak while swimming to burst forward on a jet of water. The jet costs hunger and consumes a charge. |
| Seaborne (`seaborne`) | `seaborne-brine-skin` | Brine Skin | While wet you slowly regenerate and take reduced damage. The buff lingers briefly after you dry off. |
...
| Stealth (`stealth`) | `stealth-ghost-armor` | Ghost's Armor | Slowly builds a separate armor layer while you avoid damage. It stacks beyond worn armor. The next armor-respecting hit consumes it. |
...
| Stealth (`stealth`) | `stealth-enderveil` | Enderveil | Look at Endermen freely. This prevents Enderman aggression without wearing a pumpkin. |
| Stealth (`stealth`) | `stealth-shadow-decoy` | Shadow Decoy | Stopping a sneak creates a short-lived decoy with the player's skin and equipment. Nearby mobs redirect their aggression to the decoy. The owner becomes temporarily invisible and leaves a configurable smoke trail. |
...
| Taming (`taming`) | `tame-alphas-command` | Alpha's Command | Sneak-left-click while holding a bone to raycast a target. Mark it with a private red glow. Command nearby combat pets to focus it. Successful commands consume one bone. |
...
| Unarmed (`unarmed`) | `unarmed-disarm` | Disarm | Bare-hand hits can knock the held item out of players and mobs alike. Mobs may also have a worn armor piece knocked loose. |
