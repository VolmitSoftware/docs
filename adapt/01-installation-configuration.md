...
All are soft dependencies. See [Protection & Region Policy](/adapt/08-protection-region-policy) and [Integrations](/adapt/09-integrations) for activation and failure behavior.
...
Local player JSON is authoritative only when `sql.enabled = false`. With SQL enabled, a local JSON file can seed an empty SQL record, and `.pending-sql` files preserve shutdown writes that did not reach SQL; see [Runtime Architecture](/adapt/38-runtime-architecture).
...
| `xpCurve` | `ADAPT_BALANCED` | Skill and master-level curve; see [Configuration Math](/adapt/05-configuration-math) |
...
The `[gui]` keys, icon precedence, and ordering rules are in [GUI Customization](/adapt/06-gui-customization).
...
Invalid failure-mode strings fall back to `deny` for use policies and `allow` for cost providers. See [API - Ability Use Policy](/adapt/43-api-ability-use-policy) and [API - Ability Cost](/adapt/44-api-ability-cost).
...
Redis is a one-minute handoff cache over SQL, not a storage replacement. It starts only at plugin enable; see [Velocity & Cross-Server](/adapt/39-velocity-cross-server).
...
`adaptationUsageConflicts` maps an adaptation ID to learned adaptation IDs that prevent its use. `protectionOverrides` starts from the enabled/default protector set and then includes or removes protectors by exact `Protector.getName()` values. Full protector names and defaults are in [Protection & Region Policy](/adapt/08-protection-region-policy).

### Other nested sections

| Section | Reference |
|---|---|
| `value` | [Recipes, Brewing & Value](/adapt/37-recipes-brewing-value) |
| `gui` | [GUI Customization](/adapt/06-gui-customization) |
| `farmPrevention` | [Configuration Math](/adapt/05-configuration-math) |
| `adaptationXp` | [Configuration Math](/adapt/05-configuration-math) |
| `xpIntegrity` | [Configuration Math](/adapt/05-configuration-math) |
| `permissionXpMultipliers` | [Configuration Math](/adapt/05-configuration-math) |
| `protectorSupport` | [Protection & Region Policy](/adapt/08-protection-region-policy) |
...
Durations ending in `Millis` are milliseconds; fields ending in `Ticks` use server ticks. Factors are multipliers, ranges and distances are blocks, angles are degrees, and count/charge fields are bounded during normalization. Player-facing behavior for each profile is in [Mutations Catalog](/adapt/35-mutations-catalog).
...
Use `/adapt configure` for the in-game editor, `/adapt default ...` to reset selected core/skill/adaptation TOML, and `/adapt migrate-configs` for canonical skill/adaptation rewrites and legacy JSON cleanup. Command scopes and permissions are in [Commands & Permissions](/adapt/04-commands-permissions).
