...
learn the adaptation to level 1 or higher. The Unarmed skill and that adaptation
must both be enabled in config. You need the `adapt.use` permission for it. Any
protection or region plugin must allow the action on that target. Learn adaptations from the Adapt menu (`/adapt`), under Unarmed.
