...
description: "Adapt documentation: Operator Runbooks & Smoke Tests"
published: true
...
1. Earn XP for one skill and verify its XP, level, and knowledge against the formulas in [Concepts](/adapt/02-concepts) and [Configuration Math](/adapt/05-configuration-math).
...
6. Exercise `/adapt clear` only on a disposable profile. Use `/adapt reset` only after backing up that profile and confirm the reset/default permission requirements in [Commands & Permissions](/adapt/04-commands-permissions).
...
5. If installed, test the exact surfaces in [Integrations](/adapt/09-integrations): PlaceholderAPI keys/offline expiry, Vault charge and refund failure, HiddenOre block rewards, Iris tree-feller routing, AdvancedChests Rift Access, and MagicCosmetics armor slots.
...
2. Verify discovery chance, unlock requirements, slot count, consent mode, allowed worlds, combat lock, cooldowns, resource costs, and profile-specific settings from [Mutations Overview](/adapt/34-mutations-overview) and [Mutations Catalog](/adapt/35-mutations-catalog).
...
1. Establish identical SQL/Redis settings on two disposable backends and install the companion on Velocity as described in [Velocity & Cross-Server](/adapt/39-velocity-cross-server).
