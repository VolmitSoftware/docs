...
`schemaVersion` must be exactly `1`. Any other value is a hard reject with `unsupported <kind> schemaVersion: <n>`. `<kind>` is the folder name (`holograms`, `boards`, `emoji`, `animations`, `bubbles`, `tablist`, `motd`).
...
A legacy `config.yml` is overlaid last. Its mechanical keys land in `config.toml`. Its tablist keys merge into `tablist.json`. Its `chat-bubbles` keys merge into `bubbles/default.json`. Its `motd.texts` merge into `motd.json` (each text split on newlines and truncated to the 2-line MOTD limit). The bubble and MOTD content are only applied while the target document is still byte-identical to the shipped default. A customized document is left alone. A note explains that the content was not applied. The file is then renamed to `config.yml.imported`.
